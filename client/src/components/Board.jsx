...
class Board extends Component {
  constructor(props) {
    super(props);
    this.moveSound = new SoundPlayer(src.move);
    this.captureSound = new SoundPlayer(src.capture);
  }
  ...
  render() {
    ...
    const classes = ["board", isMyTurn && !completed && "is-my-turn"]
      .filter(cls => !!cls)
      .join(" ");

    return user.id === game.white ? (
      <div className={classes}>
        {currentPosition.map((row, i) => (
      ...
      <div className={classes}>
        {positionRotated.map((row, i) => (
          <div className="row" key={i}>
            {row.map((elem, j) => {
              let coords = {
                row: positionRotated.length - 1 - i,
                col: row.length - 1 - j
              };
              return (
                <Square
