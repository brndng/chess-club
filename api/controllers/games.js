...
module.exports = {
  ...
  documentGame: async (req, res) => {
    const { id, moves, white, completed, winner } = req.body;
  ...
  resign: async (req, res) => {
    const { id, user, white, completed, winner } = req.body;
    const result = winner === white ? "1 - 0" : "0 - 1";
    if (user.id !== req.session.user) {
      res
        .status(401)
        .send(
          "No hacks allowed. Only authenticated users may resign from games."
        );
    } else {
      try {
        const record = await Game.update(
  ...
  acceptDraw: async (req, res) => {
    ...
    if (!drawOfferedByOpponent) {
      res
        .status(401)
        .send(
          "No hacks allowed. You may only accept draws if offered by the opponent."
        );
    } else {
      try {
        const record = await Game.update(
