...
class Game extends Component {
  ...
  async componentDidMount() {    
    ...
    const { id } = this.state;
    const game = await axios.get(`https://chess-club.herokuapp.com/games/${id}`);

    this.socket = await io(`https://chess-club.herokuapp.com/`);
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      const prevMoves = moves.slice(0, moves.length - 1);
        if ((user.id === white) === whiteToMove) {
          axios.put(`https://chess-club.herokuapp.com/games/move`, { 
            id,
            user, 
            game,
            selection, 
    ...
    if (_isKingInCheck && prevProps.inCheck !== user.id) {
      ...
      if(_checkMate) {
        ...
        axios.put(`https://chess-club.herokuapp.com/games/document`, { 
          id, 
          user,
          moves,
          white,
      ...
      axios.put(`https://chess-club.herokuapp.com/games/check`, { id, inCheck: user.id });
    }

    if (!_isKingInCheck && prevProps.inCheck === user.id) {
      this.socket.emit('check', { userId: null, id });
      axios.put(`https://chess-club.herokuapp.com/games/check`, { id, inCheck: null });
    }
  }
  ...
  async initOpponent(game) {
    ...
    const opponent = await axios.get(`https://chess-club.herokuapp.com/users/profile/${opponentId}`);
