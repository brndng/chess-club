...
class LogIn extends Component {
  ...
  async logIn() {
    ...
    try {
      const response = await axios.post(`https://chess-club.herokuapp.com/users/login`, { username, password });
  ...
  async signUp() {
    const { username, password } = this.state;
    try {
      const response = await axios.post(`https://chess-club.herokuapp.com/users/signup`, { username, password });
      if (response.status === 200) {
        this.setState({
