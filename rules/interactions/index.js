const isLegalMove = require("../movement/");
  ...
  for (let row = 0; row < position.length; row++) {
    for (let col = 0; col < position[row].length; col++) {
      if (position[row][col] !== null) {
        ...
        const ally =
          userId === white ? piece.toUpperCase() : piece.toLowerCase();
        const enemy =
          userId === white ? piece.toLowerCase() : piece.toUpperCase();
        const attacker = camp === "ally" ? ally : enemy;
        if (
          piece === attacker &&
          isLegalMove(attacker, { row, col }, targetSquare, position, moves)
        ) {
          isAttacked = true;
          break;
        }
...
const willMoveExposeKing = (
  ...
  if (isAlly) {
    if (
      ...
      (preview[destin.row][destin.col] !== "K" &&
        preview[destin.row][destin.col] !== "k")
    ) {
...
const getCandidateSquares = (
  ...
  for (let row = 0; row < position.length; row++) {
    for (let col = 0; col < position[row].length; col++) {
      ...
      if (
        isLegalMove(piece, origin, destin, position, moves) &&
